...
import edu.java.member.model.Member; // 다른 패키지에 있기때문에 import문을 꼭 쓸것

// 클래스나 인터페이스에서 선언된 public static 멤버 변수 이름을 import하는 방법:
import static edu.java.member.view.MainMenu.*; //*->모든것
...
// - edu.java.mamber.controller.MemberDao, MemberDaoImpl(구현클래스)
// - 인터페이스와 구현 클래스로 나눔
...
public class MemberMain {
	...
	private static MemberDao dao = MemberDaoImpl.getInstance(); // 다형성(polyorphism)

	public static void main(String[] args) {
		MemberDao dao1 = MemberDaoImpl.getInstance(); // Singleton 디자인 패턴
		MemberDao dao2 = MemberDaoImpl.getInstance(); // Singleton 디자인 패턴
	...
	private static void updateMemberInfo() {
		...
		// result = dao.update(index, password); integer,string
		// 업데이트 결과 출력
		if (result == 1) {
	...
	private static void insertNewMamber() {
		System.out.println();
		System.out.println("----- 회원 가입 화면 -----");

		System.out.println("ID 입력>>>"); // ID 입력
		String id = scanner.nextLine();
		System.out.println("Password 입력>>>"); // PW 입력
		String pwd = scanner.nextLine();
		Member member = new Member(id, pwd); // Member 객체 생성
	...
	private static void showMainMenu() {
		System.out.println();
		System.out.println("---------------------------------------------------------------------");
		System.out.println("[1] 전체검색 [2] 인덱스로 검색하기 [3] 회원가입 [4] 정보수정 [0] 종료");
		System.out.println("---------------------------------------------------------------------");
