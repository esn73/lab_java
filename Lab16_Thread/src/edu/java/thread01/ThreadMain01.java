 ...
 * 2) 상속받는 클래스에서 run()메서드를 override -> Thread가 할 일
 * 3) 상속받는 클래스의 인스턴스를 생성
 * 4) 생성된 인스턴스의 start() 메서드를 호출 -> Thread 시작.
 * (주의) Thread를 시작시킬 때 run 메서드를 직접 호출하지 않음!
 * 
 */
...
class MyThread extends Thread {
	...
	//2) run 메서드를 override - Thread가 할 일을 정의
	@Override
	public void run() {
		for (int i=0; i<100; i++) {
...
public class ThreadMain01 {
	...
	public static void main(String[] args) {
		...
		// 5) main thread가 다른 thread들의 실행이 끝날 때 까지 기다림
		try {
			th1.join(); // th1이 끝날 때 까지 main thread가 기다림
			th2.join(); // th2이 끝날 때 까지 main thread가 기다림
			th3.join(); 
		} catch (InterruptedException e) {
			e.printStackTrace();
		}
