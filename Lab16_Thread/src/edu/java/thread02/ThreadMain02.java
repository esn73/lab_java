 ...
 * 5) 4)에서 생성된 Thread (상속 클래스) 객체의 start() 메서드를 호출->Thread 시작
 * 
 */
...
public class ThreadMain02 {
	...
	public static void main(String[] args) {
		...
		class MyRunnable implements Runnable{//thread 처럼 java.lang에 있기때문에 import를 선언하지않음, 함수형이기때문에 추상메서드 하나만 가지고있음
			private String name;
			...
			public void run() {
				for(int i=1; i<= 100; i++) {
					...
					try {
						...
						//대신에, 클래스 이름으로 접근해서, Thread 클래스에서 static으로 선언된 sleep메서드 호출
						Thread.sleep(10);
		...
		Thread th4 = new Thread(new Runnable() { //inner class
		...
		Thread th5 = new Thread(() -> {
			...
			for (int i = 1; i <=100; i++) {
				System.out.println(i +  ": 안녕, Labmda!");
				try {
					Thread.sleep(10);
				} catch (InterruptedException e) {
					e.printStackTrace();
