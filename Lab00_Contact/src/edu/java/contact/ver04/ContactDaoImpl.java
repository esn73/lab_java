...
public class ContactDaoImpl implements ContactDao {
	...
	private ContactDaoImpl (){
		...
		//TODO: data 폴더에 있는 contatcs.dat 파일을 읽어서 필드 contacts를 초기화.
		contacts = initializeData();
	}
	public static ContactDaoImpl getInstance() {
		if (instance == null) {
			instance = new ContactDaoImpl(); //생성자실행
		}
		return instance;
	}
	...
	public int insert(Contact c) {
		...
		writeDatatToFile(contacts, dataFile);
		return 1;
	}
	...
	public int update(int index, Contact contact) {
		int result = 0;
		if(index >= 0 && index < contacts.size()) { // 파라미터 index가 배열의 인덱스 범위 안에 있으면ㅅ
