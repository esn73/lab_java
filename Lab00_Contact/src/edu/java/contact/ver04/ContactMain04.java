...
public class ContactMain04 {
	...
	private static void deleteCotactByIndex() {
		System.out.println();
	...
	private static int inputInteger() {
		while (true) {
			...
			try {
				...
				return n;
			} catch (NumberFormatException e) { // 에러가 발생하면 메서드가 끝나지않고 while문으로 다시 입력받음
				System.out.println("입력한 값은 정수가 아닙니다. 다시 입력하세요>>");
				// return -1; // 메뉴가 추가되지않는 잘안쓰는 숫자로 쓰는 방법 or while문을 써서 무한루프
			}
		}
	}
