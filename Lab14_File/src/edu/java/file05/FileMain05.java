 ...
 * 
 * FIS, FOS과 같은 클래스의 생성자를 호출한 이후에는 반드시 생성된 객체에서 close() 메서드를 호출
 ...
 * -> Java Runtime(JRE)이 생성된 리소스들의 close() 메서드를 자동으로 호출해줌.
...
public class FileMain05 {
	...
	public static void main(String[] args) {
		...
		try (
			...
			int b = bin.read();
			System.out.println((char) b); // 1byte 를 읽음
			b = bin.read();
			System.out.println((char) b);// 1byte를 읽음
			// BIS 객체 in의 close가 자동 호출 -> FIS 객체 in도 close됨
		} catch (Exception e) {
			e.printStackTrace();
			// BIS 객체 in의 close가 자동 호출 -> FIS 객체 in도 close됨
