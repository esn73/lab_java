...
public class FileMain12 {
	public static void main(String[] args) {
		...
		//user.hone: OS에 로그인한 사용자(User)의 기본(홈) 디렉토리 (폴더).
		String home = System.getProperty("user.home");
		System.out.println(home);
		
		//디렉토리 구분자(separator):
		//(1) MS-Windows: \
		//(2) Linux, MacOS, Unix, Android, iOSm ...: /
		//File.separotor 필드를 사용하면, Java 프로그램이 실행되는 환경에 맞춰서 \ 또는 /가 자동으로 선택됨.
		
		//절대 경로(absolute path):
		//시스템의 루트(root, C:\, /) 부터 시작해서 전체 경로 이름을 사용하는 방법.
		//(예) C: \study\lab_java\Lab14_file\data\test.txt
		//(예) /User/gildong/workspace/proj1/src/Main.java
		...
		System.out.println();
		File f2 = new File("C: " + File.pathSeparator + "study" + File.separator + "abc.txt"); //절대 경로로 생성
		System.out.println("file: " + f2);
		System.out.println("절대 경로: " + f2.getAbsolutePath());
		System.out.println("경로: " + f2.getPath());
		// 파일 객체를 생성할 때 절대 경로를 사용해서 생성한 경우에는, getPath 메서드는 절대경로를 리턴함.
