...
/*
 * 변수 선언:
 * 1. 필드/멤버 변수(filed/member variable): 클래스 안에서 선언하는 변수.-private, public
 * 		1) 인스턴스 변수: static이 아닌 멤버 변수. 
 * 			- 인스턴스를 생성한 후에 참조변수를 통해서 접근이 가능한 변수.
 * 			- 멤버 변수들은 메모리의 Heap 영역에 생성.
 * 		2) 클래스 (static) 변수 : static으로 선언된 멤버 변수.
 ...
 * 			- 외부 클래스(outer class)의 인스턴스가 생성되어야만 사용할 수 있는 클래스.
 * 		2) static 내부 클래스(중첩 클래스, nested class): static인 멤버 내부 클래스. 안드로이드 환경.
...
public class InnerMain01 {
	private int x; // 인스턴스 변수 heap - 생성자를 호출할 때 
	static int y = 0; // *클래스 (static) 멤버변수 method

	public static void main(String[] args) {
		int x = 100; // 지역 변수 -stack
		// 지역 변수는 선언되어 있고, 초기화 되어있으면 사용 가능.
